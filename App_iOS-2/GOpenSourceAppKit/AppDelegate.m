...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [GosCommon sharedInstance].controlHandler = ^(GizWifiDevice *device, UIViewController *deviceListController) {
        ...
        GosDeviceViewController *devCtrl = [[GosDeviceViewController alloc] initWithDevice:device];
    ...
    if ([APP_ID isEqualToString:@"your_app_id"] || APP_ID.length == 0 || [APP_SECRET isEqualToString:@"your_app_secret"] || APP_SECRET.length == 0) {
        ...
        if ([GosCommon sharedInstance].cloudDomainDict.count > 0) {
            [GizWifiSDK startWithAppID:APP_ID appSecret:APP_SECRET specialProductKeys:[GosCommon sharedInstance].productKey cloudServiceInfo:[GosCommon sharedInstance].cloudDomainDict autoSetDeviceDomain:NO];
        } else {
            //设置服务器
