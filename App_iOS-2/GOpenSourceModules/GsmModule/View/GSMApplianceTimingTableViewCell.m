//
//  GSMApplianceTimingTableViewCell.m
//  GOpenSource_AppKit
//
...
@implementation GSMApplianceTimingTableViewCell{
    UILabel *timeLabel;
    UILabel *socketLabel;
    UILabel *statusLabel;
    UISwitch *mySwitch;
...
-(void)initView{
    ...
    CGRect rect = [timeString boundingRectWithSize:CGSizeMake(MAXFLOAT,50)
                                   ...
                                   attributes:@{NSFontAttributeName:timeLabel.font}
                                      context:nil];
    timeLabel.frame = CGRectMake(20, 20, rect.size.width, rect.size.height);
    timeLabel.text = timeString;
    [bcview addSubview:timeLabel];
    
    socketLabel = [[UILabel alloc]initWithFrame:CGRectMake(timeLabel.frame.origin.x+timeLabel.frame.size.width+10, timeLabel.frame.origin.y, SCREENWIDTH - 80 - (timeLabel.frame.origin.x+timeLabel.frame.size.width+10), timeLabel.frame.size.height)];
    socketLabel.textColor = OFFCOLOR;
    socketLabel.font = [UIFont systemFontOfSize:15];
    socketLabel.textAlignment = NSTextAlignmentCenter;
    [bcview addSubview:socketLabel];
    
    mySwitch = [[UISwitch alloc]initWithFrame:CGRectMake(socketLabel.frame.origin.x+socketLabel.frame.size.width+10, timeLabel.frame.origin.y, 50, 30)];
    ...
    NSString *statusstring = NSLocalizedString(@"Timed_opening", nil);
    ...
    UIButton *InquireButton = [[UIButton alloc]initWithFrame:CGRectMake(weekViwe.frame.origin.x+weekViwe.frame.size.width,weekViwe.frame.origin.y-5,50, 30)];
...
-(void)switchAction:(id)sender{
    ...
    [self.dic setObject:status forKey:@"switchStatus"];
    self.clickSwitch(self.dic);
}

-(void)layoutSubviews{
    socketLabel.text = [NSString stringWithFormat:@"%ld %@",[[self.dic objectForKey:@"socketID"] integerValue] +1,NSLocalizedString(@"SOCKET", nil)];
    ...
    if ([status isEqualToString:@"0"]) {
        statusLabel.text = NSLocalizedString(@"Timed_off", nil);
    }else{
        statusLabel.text = NSLocalizedString(@"Timed_opening", nil);
    }
